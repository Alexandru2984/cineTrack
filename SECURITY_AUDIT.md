...
| Spoofing/account takeover through a stolen password, refresh token, or recovery code | Medium | Medium | Argon2id, HIBP k-anonymity, optional encrypted TOTP, second-factor-aware step-up for sensitive actions, login lockout, refresh rotation/replay detection, owner-visible security activity, session revocation, new-login and credential-change email alerts | Add passkeys. At review time none of the three live accounts had enabled TOTP. |
...
- Added an owner-scoped Security Activity timeline to web, mobile, and portable
  account exports. Inserts and the 200-row per-account cap are serialized in the
  same transaction as the protected action, the API returns at most the latest
  100 rows with `no-store`, and the retention sweep deletes rows after 90 days.
  Successful password logins, password changes/resets, and TOTP changes now send
  non-actionable email notices through the existing monitored SMTP path.
- Corrected the privacy/subprocessor disclosures and the incident-notification
  decision tree in English and Romanian.
- Validation passed strict formatting/Clippy, 281 backend unit tests, all 111
  PostgreSQL integration tests, frontend ESLint, all 151 frontend tests,
  TypeScript, the production PWA build, mobile lint and TypeScript, all 150
...
2. **Account security center (completed in code):** second-factor-aware step-up,
   new-login/credential-change email alerts, a bounded owner-visible activity
   timeline, and one-tap session revocation are implemented. Production rollout
   remains manual and pending operator push/deploy.
...
- DB backup to R2 (`scripts/backup_to_r2.sh`): consistent custom-format `pg_dump`, archive validation, optional client-side age encryption, verified SHA-256 upload, retention (14 days), stable cron locking, and Prometheus status metrics. `scripts/restore_from_r2.sh` validates downloads and protects existing/production restore targets with explicit guards.
...
- Mobile offline query persistence now keeps only core library/calendar/media data, encrypts the AsyncStorage payload with AES-256-GCM using a device-only SecureStore key, removes legacy plaintext cache, and offers an explicit local-data clear action. Exact history, lists, statistics, social, notifications, and account data are not persisted.
- Unsigned EAS Update remains available only to internal preview builds. Production/store builds disable OTA until end-to-end update signing is available, so the store-reviewed embedded bundle is the production trust boundary.
- CI now parses generated Android/iOS security configuration and compiles an Android release, closing the gap where Metro export could pass while native modules or permissions were broken. A separate operations job runs ShellCheck, syntax-checks embedded Python, exercises backup/restore guards with isolated fakes, and validates Prometheus rules.
...
- ~~Access tokens stay stateless until they expire.~~ **Closed 2026-08-18.** Access tokens now carry a `sid` claim naming the refresh-token family, and every revocation path — logout, `logout-all`, session revoke, password change, password reset, and refresh-reuse detection — records the revocation in `access_token_revocations` and mirrors it into an in-process cache checked on every authenticated request. Revocation is immediate rather than bounded by the token lifetime. The cache is rebuilt from the table at startup, so a restart cannot resurrect a revoked session; entries expire after 75 minutes, beyond the 60-minute ceiling `JWT_EXPIRY_MINUTES` allows. Tokens minted before the claim existed stop validating at deploy, which both clients absorb through their existing 401-refresh path. See `backend/src/services/revocation.rs`.
...
- ~~`/metrics` has no authentication.~~ **Closed 2026-08-18.** The endpoint now requires a bearer token, compared in constant time, and `METRICS_BEARER_TOKEN` is mandatory in production — startup fails without it rather than serving an open endpoint. Prometheus reads the credential from a mode-640 file written by `scripts/render_metrics_token.sh`, so nothing is committed. The previous reasoning ("nginx does not proxy it") was sound for a dedicated host but not for this one, where the backend port is published on `127.0.0.1:8090` alongside roughly twenty unrelated containers.
