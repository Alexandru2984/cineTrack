...
fn generate_salt() -> Result<[u8; Salt::RECOMMENDED_LENGTH], AppError> {
...
mod tests {
    ...
    fn generated_salts_are_unique_and_recommended_width() {
        ...
        assert_eq!(first.len(), Salt::RECOMMENDED_LENGTH);
        assert_ne!(first, [0u8; Salt::RECOMMENDED_LENGTH]);
    }
    ...
    fn verifies_a_hash_whose_salt_this_code_did_not_generate() {
        ...
        let legacy = Argon2::default()
            .hash_password_with_salt(b"Passw0rd123!", b"abcdefghijklmnop")
    ...
    /// A hash this codebase actually produced, checked in as a literal.
    ///
    /// Every other test here hashes and verifies with the same build, so a
    /// crate upgrade that changed the encoding on both sides would pass all of
    /// them and lock every existing account out on deploy. This one is the only
    /// thing standing between a dependency bump and that outcome: the string
    /// below was generated by argon2 0.5.3 under the parameters in production,
    /// and it must keep verifying whatever the crate does next.
    ///
    /// If this fails after a version bump, do not regenerate it. It means
    /// stored passwords no longer verify, and the upgrade needs a migration
    /// path rather than a new fixture.
    #[test]
    fn a_hash_stored_by_an_earlier_release_still_verifies() {
        const STORED: &str = "$argon2id$v=19$m=32768,t=3,p=1$5EeKcz+7vyTL+cs2hFdkHg\
                              $PdoHh7dt8/4gYi9I1Y5UeKtjonhoEABig/F7WRRWYd0";
        let stored: String = STORED.split_whitespace().collect();

        assert!(verify_password_sync("Pass1234", &stored).expect("verify"));
        assert!(!verify_password_sync("Wr0ngPass!", &stored).expect("verify"));
    }

    /// The same guarantee for the cost most stored hashes actually use.
    ///
    /// The fixture above was written at the current parameters, but raising the
    /// cost never rewrote the hashes already in the table: production holds
    /// nine accounts at `m=19456,t=2,p=1` against two at the current
    /// `m=32768,t=3,p=1`. A crate upgrade that only kept faith with the newer
    /// shape would lock out most of the accounts and pass the other test.
    ///
    /// Generated by argon2 0.5.3 at the older cost. Same rule: if it fails
    /// after a version bump, that is a migration problem, not a stale fixture.
    #[test]
    fn a_hash_stored_at_the_previous_cost_still_verifies() {
        const STORED: &str = "$argon2id$v=19$m=19456,t=2,p=1$bGVnYWN5c2FsdDE2Ynl0\
                              $1k4WJUqink5oU8FFf+Nvbj6l6DK3OKYuDgTeFb5VC9Q";
        let stored: String = STORED.split_whitespace().collect();

        let params = Params::try_from(&PasswordHash::new(&stored).expect("parse")).expect("params");
        assert_ne!(
            params.m_cost(),
            ARGON2_MEMORY_KIB,
            "this no longer covers a cost different from the configured one"
        );
    ...
    fn passwords_hashed_at_the_previous_cost_still_verify() {
        ...
        let legacy = Argon2::default()
            .hash_password_with_salt(b"Pass1234", &salt)
            .expect("legacy hash")
            .to_string();
