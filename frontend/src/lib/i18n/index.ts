...
/**
 * Resolve a dot-path translation key for a locale. Falls back to English and
 * then to the raw key, so a not-yet-translated string degrades to readable
 * English rather than blanking the UI. `{name}` placeholders are interpolated
 * from `params`.
 */
