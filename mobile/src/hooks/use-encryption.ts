...
export function useKeyStatus() {
  ...
  return useQuery<KeyStatus>({
    ...
    queryFn: () => apiRequest<KeyStatus>('/encryption/keys'),
 ...
 *
 *  The three outcomes it distinguishes — no keys anywhere, keys elsewhere but
 *  not here, keys here — are what every messaging screen branches on, and
 *  working them out per screen would give different answers in different
 *  places. */
export function useEncryptionSession() {
  ...
  useEffect(() => {
    if (!hasLocalSession(authStatus) || !userId) {
      setStatus('loading');
      return;
    }
    if (!status) return;
  ...
  }, [authStatus, userId, status, setIdentity, setStatus]);

  useEffect(() => {
    if (hasLocalSession(authStatus)) return;
    void clear(null);
  }, [authStatus, clear]);
}
...
export function useRestoreEncryption() {
  ...
  return useMutation({
    mutationFn: async ({ secret, kind }: { secret: string; kind: 'password' | 'recovery' }) => {
      if (!userId) throw new Error('not-authenticated');
